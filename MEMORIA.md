...
## Calendario turbinado + sync bidirecional + gestao (2026-07-12)
- BUG DA NOTA que nao gravava: era o LIMITE de 12 funcoes da Vercel (o antigo /api/pipedrive-nota era a 13a e dava 404). Consolidado em pipedrive-acao ja resolveu. Alem disso, SAFE_MODE estava "" e o CLI nao gravava "false"; entao inverti o codigo: escrita LIGADA por padrao, modo seguro so se PIPEDRIVE_SAFE_MODE === "true" (em enviar-pipedrive e pipedrive-acao).
- pipedrive-acao ganhou acoes: atividade-feita (PUT /activities/{id} done) e atividade-nova (POST /activities). pipedrive-deal devolve id das atividades (mapA). Tela Negocio: bolinha pra concluir atividade + form "nova atividade" (assunto+data) gravando no Pipedrive; proximas x atrasadas ja separadas.
- CALENDARIO detalhado: evento expansivel (clica pra abrir) mostra CONVIDADOS com bolinha de status (accepted=verde/declined=vermelho/tentative=amarelo/needsAction=cinza, organizador marcado), DESCRICAO, e LINK da reuniao (Meet via hangoutLink/conferenceData + "Abrir no Google Calendar" via htmlLink). eventos.js enriquecido (descricao limpa de HTML, status, organizador, link, htmlLink).
- GESTAO: cancelar evento no Google (criar-evento.js agora aceita DELETE, sendUpdates=all avisa participantes) via botao lixeira no Calendario com modal. Excluir reuniao no AtaLead (db.excluirReuniao, cascade cuida de participantes/atas/passos, remove prospect orfao; nao mexe no Pipedrive) via lixeira na lista Reunioes com modal.
- EXPORT Excel real (.xlsx) na tela Reunioes com exceljs (import dinamico, chunk separado): tabela com grade so nela (showGridLines:false + bordas nas celulas), cabecalho verde (FF0E7C74) branco, tudo centralizado, largura por conteudo. Colunas: Data, Titulo, Empresa, Contato, Participantes, Proximos passos, Status, No Pipedrive, Valor estimado.
- SYNC BIDIRECIONAL calendario<->AtaLead (escolha do usuario: registrar todo evento + criar/cancelar): migration 20260712000001 add coluna reunioes.google_event_id. db.sincronizarAgenda(eventos) roda ao abrir o Calendario (e no botao Sincronizar): insere eventos que faltam como status 'agendada', e marca 'cancelada' os que sumiram do Google na janela -7d/+30d. criarReuniaoCompleta aceita googleEventId e ATUALIZA o registro 'agendada' (nao duplica) quando a ata vem de um evento (NovaReuniao passa eventoId). Reunioes mostra pill de status (Ata gerada/Agendada/Cancelada), clique em 'agendada' vai pro Calendario. Botao Sincronizar (icone refresh) le o Google ao vivo e re-sincroniza. Nao ha sync automatico em background (Vercel Hobby sem cron) - e on-open/on-demand.
- NOTA: registrar TODO evento pode encher Reunioes de blocos internos (Break/Almoco). Se incomodar, filtrar depois.

## Modelos de ata por tipo + mapa de falantes + analise de venda (2026-07-12)
- MAPA DE FALANTES: NovaReuniao detecta "Speaker/Orador/Falante N" na transcricao (detectarSpeakers) e mostra um seletor por falante pra ligar ao participante; manda mapa EXPLICITO pra Tess (campo falantes em gerar-ata; fallback posicional). Botao "+ Adicionar participante" alimenta o seletor (p/ quem entrou sem convite). Dedução de empresa por e-mail agora ignora provedores genericos (gmail/hotmail/etc.) - deixa em branco (NovaReuniao empresaDoEmail + eventos.js empresaDe).
- MODELOS DE ATA (tipos de reuniao): tabela modelos_ata (migration ...002, seed: Prospecção/Follow-up/Demonstração/Interna; RLS auth all). Seletor "Tipo de reunião" na NovaReuniao + botao "Modelos" abre modal (GerenciarModelos) que cria/edita/exclui (db.listarModelos/criarModelo/atualizarModelo/excluirModelo). O modelo escolhido manda instrucoes pra Tess (foco); o FORMATO de saida (JSON) continua o mesmo pra nao quebrar lead/passos.
