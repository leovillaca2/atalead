...
// Guarda global: se a Nova reunião tem transcrição não gerada, confirma antes de sair.
function podeSair() {
  if (window.__ataleadDirty && !window.confirm("Você tem uma transcrição que ainda não virou ata. Sair mesmo assim?")) return false;
...
function Sidebar({ email }) {
  ...
  return (
    <aside className="sidebar">
      ...
      {items.map((it) => (
        <button key={it.to} className={"navitem" + (it.match(pathname) ? " active" : "")} onClick={() => { if (podeSair()) nav(it.to); }}>
          <Icon name={it.icon} size={17} /><span>{it.name}</span>
        </button>
      ))}
      <div className="nav-label mt">SISTEMA</div>
      <button className="navitem" onClick={() => { if (podeSair()) supabase?.auth.signOut(); }}><Icon name="logout" size={17} /><span>Sair</span></button>
  ...
  return (
    <div className="topbar">
      ...
      <div className="top-actions">
        <button className="btn" onClick={() => { if (podeSair()) nav("/nova"); }}><Icon name="plus" size={15} strokeWidth={2.2} /><span>Nova reunião</span></button>
